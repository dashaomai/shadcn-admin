...
export namespace Protocol {
  ...
  export function isCompressed(buffer: Uint8Array): CompressType {
    if (buffer.length > 2) {
      ...
      if (
        d0 === 0x78 &&
        (d1 === 0x9c || d1 === 0x01 || d1 === 0xda || d1 === 0x5e)
      ) {
...
export namespace Message {
  ...
  export function encode(
    ...
    if (msgHasRoute(type)) {
      offset = encodeMsgRoute(
        compressRoute,
        compressRoute ? (route as number) : routeBytes!,
        buffer,
        offset
      )
    }
  ...
  export function encodeMsgRoute(
    compressRoute: boolean,
    route: Uint8Array | number,
    buffer: Uint8Array,
    offset: number
  ): number {
    if (compressRoute) {
