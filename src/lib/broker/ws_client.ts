  ...
  export class WebSocketConnector {
    ...
    private startHeartbeat(): void {
      ...
      this.heartbeatTimeoutId = setInterval(() => {
        ...
        if (
          this.socket &&
          this.isConnected &&
          Date.now() - this.lastTime > this.heartbeatTimeout
        ) {
          this.disconnect()
        }
      }, this.gapThreshold)
    ...
    public connect(params: ConnectorOptions, url: string): void {
      this.maxReconnectAttempts =
        ...
        WebSocketConnector.DEFAULT_MAX_RECONNECT_ATTEMPTS
    ...
    private messageDecode(data: ArrayBufferLike): MessageData | undefined {
      ...
      try {
        ...
        logger.error(
          'default decode  time:, err:, routeNum:, bodyStr:',
          newData.toLocaleString(),
          error,
          route,
          String(msg.body)
        )
