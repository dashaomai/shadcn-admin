...
export function PublishersTable({ columns, data, total }: Props) {
  ...
  const allTypes = useMemo<SelectOption<number>[]>(
    () =>
      PublisherTypeDescriptions.map((desc, i) => ({
        label: t(`apps.publishers.properties.type.${desc}`),
        value: i,
      })),
    []
  )
  ...
  const allStatus = useMemo<SelectOption<number>[]>(
    () =>
      PublisherStatusDescriptions.map((desc, i) => ({
        label: t(`apps.publishers.properties.status.${desc}`),
        value: i,
      })),
    []
  )
  ...
  return (
    <div className='space-y-4'>
      <TableToolbar
        table={table}
        placeholder={t('apps.publishers.toolbar.placeholder')}
      >
        <div className='flex gap-x-2'>
          {table.getColumn('type') && allTypes && (
            <TableFacetedFilter
              column={table.getColumn('type')}
              title={t('apps.publishers.properties.type.title')}
              options={allTypes}
            />
          )}
          ...
          {table.getColumn('status') && allStatus && (
            <TableFacetedFilter
              ...
              title={t('apps.publishers.properties.status.title')}
              options={allStatus}
            />
          )}
        </div>
